...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("P_CapitalShipCon")))
);
ResourceId constant P_CapitalShipConfigTableId = _tableId;
...
library P_CapitalShipConfig {
   ...
   */
  function set(uint8 resource, uint256 initialCost) internal {
    bytes memory _staticData = encodeStatic(resource, initialCost);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(uint8 resource, uint256 initialCost) internal {
    bytes memory _staticData = encodeStatic(resource, initialCost);
  ...
  function set(P_CapitalShipConfigData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.resource, _table.initialCost);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint8 resource, uint256 initialCost) {
    resource = (uint8(Bytes.slice1(_blob, 0)));

    initialCost = (uint256(Bytes.slice32(_blob, 1)));
  }
  ...
  function decode(
    ...
    (_table.resource, _table.initialCost) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(uint8 resource, uint256 initialCost) internal pure returns (bytes memory) {
    return abi.encodePacked(resource, initialCost);
  }
