...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("FleetStance")))
);
ResourceId constant FleetStanceTableId = _tableId;
...
library FleetStance {
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (uint8 stance, bytes32 target) {
    stance = (uint8(Bytes.slice1(_blob, 0)));

    target = (Bytes.slice32(_blob, 1));
  }
   ...
   */
  function encode(uint8 stance, bytes32 target) internal pure returns (bytes memory, PackedCounter, bytes memory) {
    bytes memory _staticData = encodeStatic(stance, target);
