...
```solidity filename="/contract-source/codegen/tables/AsteroidCount.sol" showLineNumbers {4}
...
```solidity filename="/contract-source/codegen/tables/AsteroidCount.sol" showLineNumbers {5}
library AsteroidCount {
  // ...
...
For example, in the `mud-config.ts` of the [`building-upgrade-bounty`](../world-extension/examples.mdx) world extension, the core `Position` and `OwnedBy` tables are [referenced](https://github.com/primodiumxyz/developers/blob/bca59e31bce35ce3b9feada4ee42b0259b5e371a/examples/building-upgrade-bounty/packages/contracts/mud.config.ts#L21-L35) as follows:
