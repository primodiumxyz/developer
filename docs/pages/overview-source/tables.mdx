...
For example, in the `mud-config.ts` of the [`BuildingUpgradeBounty`](../world-extension/examples.mdx) world extension, the core `Position` and `OwnedBy` tables are [referenced](https://github.com/primodiumxyz/developer/blob/bca59e31bce35ce3b9feada4ee42b0259b5e371a/examples/BuildingUpgradeBounty/packages/contracts/mud.config.ts#L21-L35) as follows:
...
```solidity copy filename="/examples/BuildingUpgradeBountyackages/contracts/src/libraries/LibHelpers.sol" showLineNumbers {1,7}
