...
**Prototype Generation** is Primodium's internal process for populating [prototype tables](./tables.mdx#prototype-tables) with [prototype configuration](./configuration.mdx) data. This page describing an example workflow used internally for creating a new ore distribution map and writing to a terrain [prototype table](./tables.mdx#prototype-tables).
...
```tsx filename="/contract-source/ts/terrain/terraingen.ts#L19-L28" copy showLineNumbers
...
```ts filename="/contract-source/ts/terrain/terraingen.ts" showLineNumbers
...
```solidity filename="/contract-source/src/codegen/scripts/CreateTerrain.sol" copy showLineNumbers
