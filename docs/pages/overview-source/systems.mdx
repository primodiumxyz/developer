...
First, locate the [`IUpgradeBuildingSystem`](https://github.com/primodiumxyz/developer/blob/main/contract-source/src/codegen/world/IUpgradeBuildingSystem.sol) interface in the `codegen/world` directory. Locate the function signature of the function you want to use, which is `upgradeBuilding()`, and add to both a contract with the same function signature and an interface in the world extension.

```solidity filename="/examples/building-upgrade-bounty/packages/contracts/src/interfaces/IUpgradeBuildingSystem.sol" showLineNumbers {6}
...
```solidity filename="/examples/building-upgrade-bounty/packages/contracts/src/interfaces/interfaces.sol" showLineNumbers {5}
pragma solidity >=0.8.21;
import { PositionData } from "../codegen/index.sol";
