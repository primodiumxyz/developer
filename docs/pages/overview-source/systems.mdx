...
First, locate the [`IUpgradeBuildingSystem`](https://github.com/primodiumxyz/developer/blob/main/contract-source/codegen/world/IUpgradeBuildingSystem.sol) interface in the `codegen/world` directory. Locate the function signature of the function you want to use, which is `upgradeBuilding()`, and add to both a contract with the same function signature and an interface in the world extension.
