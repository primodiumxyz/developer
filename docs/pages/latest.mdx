...
- All asteroids have an `Encryption` attribute to them that prevents
  colonization.
- Units are now organized into `Fleets`, which are groups of units that stay in
  orbit.
- Fleets can be used to attack other fleets in orbit or defend an asteroid from
  orbit.
- Added miners for kimberlite, titanium, iridium, and platinum for use on other
  asteroids.
...
- Major UI elements have been moved to `Widgets` which can be locked/unlocked,
  dragged, and pinned.
- Introducing Aura, your personal gaming companion. Aura provides a centralized
  location for managing widgets and key game actions and information.
