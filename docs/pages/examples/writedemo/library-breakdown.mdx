...
```solidity filename="examples/WriteDemo/packages/contracts/src/systems/WriteDemoSystem.sol" startsWith={58}
PositionData memory position = getTilePosition(asteroidEntity, building);
```

The [`getTilePosition()`](https://github.com/primodiumxyz/developer/tree/main/examples/WriteDemo/packages/contracts/src/systems/WriteDemoSystem.sol) function dives progressively deeper into helper functions. Many of these will be included in a future Primodium library package, but they are a good source of insight as well.
