...
Not much changes from ReadDemo.  We need an addition step in setUp() to delegate permission for the system to act on behalf of the user, but the rest is largely the same.

We will highlight the key differences in the Explanation below.

```ts filename="WriteDemoSystem.t.sol"
