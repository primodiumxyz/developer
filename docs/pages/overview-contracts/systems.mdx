...
In TypeScript, a system is called using the `write$` observable. First, a `worldContract` object is initialized as follows, where `getContract()` is defined [here](https://github.com/latticexyz/mud/tree/main/packages/common/src/getContract.ts#L50C17-L50C28) in the `@latticexyz/mud` package.
...
Then, a system can be called as follows as a [writeContract](https://viem.sh/docs/contract/writeContract.html) call in [`viem`](https://github.com/wevm/viem), with types defined [here](https://github.com/wevm/viem/blob/6eef194a05016f3c2967cd6b4f730012adcbfffc/src/actions/getContract.ts#L430).
...
Primodium systems can also be called from the CLI with the publicly-available ABIs, which are accessible from the latest [version information](../latest.mdx).
