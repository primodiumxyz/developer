...
A TypeScript-based tRPC server for Solana, providing protected API endpoints for
building and sponsoring user transactions.
...
2. If Redis is not installed, make sure that `NODE_ENV` is set to `local` in the
   ...
   `pnpm install`. Refer to `prepare` script in `./package.json` for details.
