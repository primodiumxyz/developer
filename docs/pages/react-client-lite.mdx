...
This template provides a minimal setup to start working on a read-only client for Primodium via our [GraphQL Endpoint](./creating-dashboards.mdx). Visit the demo site [here](https://template-react-client-lite.vercel.app/).

![Template](../public/tooling/react-client-lite/template.png)
...
The Primodium `react-client-lite` template is based on `react+vite`. It uses [`gql.tada`](https://gql-tada.0no.co/) for query authoring and `shadcn/ui` for UI elements. Source for the template is available on Github [here](https://github.com/primodiumxyz/create-primodium/tree/main/template-react-client-lite).
...
```js filename=".eslintrc.cjs" showLineNumbers {13-18} copy
module.exports = {
  // ...other rules
  parserOptions: {
    ecmaVersion: "latest",
