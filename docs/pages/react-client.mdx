...
provides scaffolding for developers to create cystom Primodium clients with the
[Primodium Core](./core-library.mdx) TypeScript API. Visit the demo site
...
- `react-client`: for building custom clients with the
  [Primodium Core Library](./core-library.mdx)
