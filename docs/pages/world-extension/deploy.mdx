...
Primodium is currently deployed on an [OP Stack](https://docs.optimism.io/stack/getting-started) testnet hosted by [Caldera](https://www.caldera.xyz) that resolves to the [Ethereum Sepolia testnet](https://sepolia.dev/).

## Forewarning

Once you deploy your Primodium world extension to the Primodium testnet, it will be live and accessible to the public. Additionally, that namespace name will become taken and no longer available to be used by anyone else (or reregistered by you). Be sure to [test your extension](./testing.mdx) on the local development environment before deploying to the live Primodium world.
...
1. Set the `worlds.json` file to the following "10017" chainID and substitute in the world address, available on the [latest release page](../latest.mdx).
